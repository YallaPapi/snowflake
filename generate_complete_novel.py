...
Generates a full novel using the Snowflake Method (Steps 0-10)
Genre: Thriller/Romance - "Code of Deception"
Target: 50,000 words
"""
...
def print_step_info(step_num, name, description):
...
def check_manuscript_quality(manuscript_path):
    """Check the quality of the generated manuscript"""
    ...
    try:
        ...
        checks = {
            ...
            "Has chapters": "Chapter" in content,
            "Has dialogue": '"' in content or "'" in content,
            "Minimum length": word_count >= 10000,  # At least 10k words
        }
        
        return True, checks
    except Exception as e:
        return False, f"Error reading manuscript: {e}"
...
def main():
    ...
    print("Method: Complete Snowflake Pipeline (Steps 0-10)")
    ...
    if manuscript_md.exists():
        ...
        success, result = check_manuscript_quality(manuscript_md)
        if success:
            print("[QUALITY] Manuscript quality checks:")
            for check, value in result.items():
                if check == "Word count":
                    print(f"   {check}: {value:,}")
                else:
                    print(f"   {check}: {'[OK]' if value else '[FAIL]'}")
        else:
            print(f"[ERROR] Quality check failed: {result}")
    else:
