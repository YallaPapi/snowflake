#!/usr/bin/env python3
"""
Final Novel Generation Script - All Issues Fixed
...
def monitor_progress(pipeline, project_id):
    """Monitor and display pipeline progress - ASCII only"""
...
def run_pipeline_with_retry(pipeline, initial_brief, story_brief, target_words, max_attempts=3):
    """Run pipeline with retry on validation failures"""
    for attempt in range(max_attempts):
        print(f"\n[ATTEMPT {attempt + 1}] Starting pipeline generation...")
        ...
        try:
            ...
            if success:
                print(f"[SUCCESS] Pipeline completed on attempt {attempt + 1}")
                return True
            else:
                print(f"[RETRY] Pipeline failed on attempt {attempt + 1}, retrying...")
                
        except Exception as e:
            print(f"[ERROR] Pipeline error on attempt {attempt + 1}: {e}")
            
        # Wait before retry
        if attempt < max_attempts - 1:
            print(f"[WAIT] Waiting 5 seconds before retry...")
            time.sleep(5)
    
    print(f"[FAILED] Pipeline failed after {max_attempts} attempts")
    return False
...
def main():
    ...
    print("Validation: Enhanced with retries")
    ...
    # Define story concept (improved to pass validation)
    story_brief = """
    A brilliant cybersecurity expert discovers her company is being used as a front for international espionage.
    When she tries to expose the truth, she becomes the target of a deadly conspiracy led by her own CEO.
    ...
    if manuscript_md.exists():
        ...
        try:
            ...
            print(f"[QUALITY] Content length: {len(content):,} characters")
    ...
    # Show what we have even if pipeline failed
    if not success:
        print_section("PARTIAL RESULTS")
        print("[INFO] Pipeline did not complete fully, but checking partial results...")
        
        # List what artifacts were created
        if project_path.exists():
            artifacts = list(project_path.glob("step_*.json"))
            print(f"[PARTIAL] Created {len(artifacts)} artifacts:")
    ...
    # Export to additional formats (only if we have a complete manuscript)
...
if __name__ == "__main__":
    try:
        ...
        if success:
            print("\n[FINAL] SUCCESS! Novel generation completed successfully!")
            print("[FINAL] Check the artifacts directory for your complete novel.")
        else:
            print("\n[FINAL] PARTIAL SUCCESS - Some artifacts were created but pipeline did not complete.")
            print("[FINAL] Check the artifacts directory for partial results.")
